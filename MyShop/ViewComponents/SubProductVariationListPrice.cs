...
{
    [ViewComponent]
    public class SubProductVariationListPrice : ViewComponent
    {
        private readonly ApplicationDbContext _db;
        public SubProductVariationListPrice(ApplicationDbContext db)
        {
            _db = db;
        }
        ...
        
        {
            var subProVars = await _db.SubProductVariations.AsNoTracking()
                .Where(c => c.ProductVariationId.Equals(productVariationId) && c.HasDefinedDifferentPrice)
                .ToListAsync();
            ...
            return View(viewName:"SubProductVariationListPrice",model: subProVars);
